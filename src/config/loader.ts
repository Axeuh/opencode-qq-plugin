...
let config: QQPluginConfig | null = null;
let configPath: string | null = null;

/**
 * 初始化配置加载器
 */
export function initConfig(configFilePath?: string): QQPluginConfig {
  // 确定配置文件路径
  ...
  config = JSON.parse(configContent) as QQPluginConfig;
 ...
 * 获取配置
 */
export function getConfig(): QQPluginConfig {
  if (!config) {
    return initConfig();
  }
  return config;
 ...
 * 重新加载配置
 */
export function reloadConfig(): QQPluginConfig {
  if (!configPath) {
    return initConfig();
  }
