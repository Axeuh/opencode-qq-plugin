...
export interface FileInfo {
  type: string;          // file, image, voice, video, forward 等
  filename: string;
  fileId: string;
  fileSize: string;
  originalCQ: string;    // 原始 CQ 码
  params: Record<string, string>;  // 所有原始参数
 ...
 */
export function parseCQCode(cqCode: string): ParsedCQCode {
  const result: ParsedCQCode = { type: '', params: {} };
 ...
 */
export function extractFileInfo(rawMessage: string): FileInfo[] {
 ...
 */
export function extractQuotedMessageId(message: any): string | null {
  try {
    // 格式1: OneBot 消息段数组
    if (message.message && Array.isArray(message.message)) {
      for (const segment of message.message) {
