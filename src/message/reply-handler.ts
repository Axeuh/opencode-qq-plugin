...
import { extractPlainText, extractFileInfo, FileInfo } from './parser';
import { getFileHandler } from './file-handler';
import { getNapCatClient } from '../napcat/client';

export interface ReplyContent {
  text: string;           // 引用消息的文本内容
  files: string[];        // 处理后的文件信息（包含下载路径）
 ...
 * 处理 raw.records 中的文件信息（NapCat 特有格式）
 */
async function processRecordsFiles(
  records: any[],
  userId: number,
  groupId?: number,
  directory?: string
 ...
 * 格式化引用消息内容
 */
export function formatReplyContent(replyContent: ReplyContent): string {
  const parts: string[] = [];
