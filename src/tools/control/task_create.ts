...
import { getTaskScheduler, TaskScheduleConfig } from '../../task/scheduler';
  ...
  async execute(args) {
    ...
    try {
      ...
      if (args.scheduleType === 'delay') {
        const config = args.scheduleConfig as any;
        task = scheduler.createDelayTask(
          args.userId,
          args.sessionId,
          args.name,
          args.prompt,
        ...
        const config = args.scheduleConfig as any;
        task = scheduler.createScheduledTask(
          args.userId,
          args.sessionId,
          args.name,
          args.prompt,
