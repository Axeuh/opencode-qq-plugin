...
import { initConfig, isUserWhitelisted, isGroupWhitelisted, getOpenCodeConfig, getBotConfig, getWhitelistConfig, getWebServerConfig, getInstanceLockConfig } from './config/loader';
import { getAgentModelCache } from './cache/agent-model';
import { getTaskScheduler } from './task/scheduler';
// QQ 消息工具
import { qqSendMessageTool, qqGetFriendsTool, qqGetGroupHistoryTool, qqSearchFriendTool, qqSendFileTool, qqGetLoginInfoTool, qqSendPokeTool, qqGetFileTool } from './tools';
// 控制工具 (参考 axeuh-control skill)
import { userSessionListTool, userSessionCreateTool, userSessionSwitchTool, userSessionDeleteTool, sessionTitleSetTool, setUserSessionCreateClient, setSessionTitleSetClient, taskListTool, taskCreateTool, taskDeleteTool, agentListTool, agentGetTool, agentSetTool, modelListTool, modelGetTool, modelSetTool, systemHealthTool, systemReloadTool, } from './tools';
import { createLogServer, interceptConsole } from './log-server';
 ...
 */
function tryAcquireInstanceLock(port) {
 ...
 * OpenCode QQ Plugin
 */
export const QQPlugin = async (ctx) => {
    ...
    if (isPrimary) {
        ...
        if (webServerConfig.enabled) {
            ...
            setOpenCodeClient(client, async (simMsg) => {
                const msg = {
                    time: Date.now(),
    ...
    // 处理 QQ 消息
    const handleQQMessage = async (msg) => {
    ...
    // 处理命令
    const handleCommand = async (messageType, groupId, userId, commandText) => {
 ...
 * 格式化消息发送到 OpenCode
 */
function formatMessageForOpenCode(text, messageType, groupId, userId, senderName) {
 ...
 * 创建 OpenCode 会话
 */
async function createOpenCodeSession(client, userId, customTitle) {
    try {
        const body = {};
        if (customTitle) {
 ...
 * 处理 /stop 命令 - 打断当前会话
 */
async function handleStopCommand(client, userId) {
    const sessionManager = getSessionManager();
 ...
 * 处理 /undo 命令 - 撤销上一条消息
 */
async function handleUndoCommand(client, userId) {
    const sessionManager = getSessionManager();
    const session = sessionManager.getUserSession(userId);
    if (!session) {
        return '没有活动的会话';
 ...
 */
async function handleRedoCommand(client, userId) {
    const sessionManager = getSessionManager();
    const session = sessionManager.getUserSession(userId);
    if (!session) {
        return '没有活动的会话';
 ...
 */
async function handleCompactCommand(client, userId) {
    const sessionManager = getSessionManager();
    const session = sessionManager.getUserSession(userId);
    if (!session) {
        return '没有活动的会话';
 ...
 */
async function handleSlashCommand(client, userId, args) {
    const sessionManager = getSessionManager();
    // 显示帮助
 ...
 * 处理 session 命令
 */
async function handleSessionCommand(client, userId, args) {
    const sessionManager = getSessionManager();
 ...
 * 处理 agent 命令
 */
function handleAgentCommand(userId, args) {
    const opencodeConfig = getOpenCodeConfig();
    const sessionManager = getSessionManager();
 ...
 * 处理 model 命令
 */
function handleModelCommand(userId, args) {
    const opencodeConfig = getOpenCodeConfig();
    const sessionManager = getSessionManager();
    const cache = getAgentModelCache();
    // 从缓存获取模型列表
