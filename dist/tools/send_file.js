...
export const qqSendFileTool = tool({
    ...
    async execute(args, context) {
        ...
        try {
            ...
            if (args.messageType === 'private') {
                result = await napcat.sendPrivateFile(parseInt(args.targetId), fileParam, fileName);
            }
            else {
                result = await napcat.sendGroupFile(parseInt(args.targetId), fileParam, fileName);
            }
            return JSON.stringify({
                success: true,
